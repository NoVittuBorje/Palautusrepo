...
blogiRouter.post('/:id', async (request,response) => {
  const body = request.body
  console.log(body,'incoming to Post')
  const newblog = {
    title:body.title,
    author:body.author,
    url:body.url,
...
blogiRouter.get('/', async (request, response) => {
  const blogs = await Blog.find({}).populate('user',{ username: 1, name: 1, id: 1 ,comments: 1})
  response.json(blogs)
})
