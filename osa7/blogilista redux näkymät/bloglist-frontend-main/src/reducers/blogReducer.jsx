...
export const AddComment = content => {
  return async dispatch => {
    ...
    const newBlog ={
      ...
      id:content.blog.id,
      likes:content.blog.likes,
      title:content.blog.title,
      url:content.blog.url,
      user:content.blog.user,
      comments:[...content.blog.comments,content.comment]
    }
    console.log(newBlog,'blog in addcomment')
    blogsService.addcomment(newBlog).then(res => {
      blogsService.getAll().then(blogs => {
        dispatch(setBlogs(blogs))
      })
    })
